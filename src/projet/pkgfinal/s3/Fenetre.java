...
public class Fenetre extends javax.swing.JFrame {
    ...
    EnsembleFigure ensembleFigure1 = new EnsembleFigure();
    EnsembleFigure ensembleFigure2 = new EnsembleFigure();
    int test = 0;
    ...
    private ArrayList<Segment> Trarr = new ArrayList<>();
    private ArrayList<Segment> Polygarr = new ArrayList<>();
    private ArrayList<Segment> Polylarr = new ArrayList<>();
    ...
    private void displayPanelMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_displayPanelMouseReleased
        if (click.isSelected()) {
            ...
            switch (ajouteFigure) {
                ...
                case "Triangle":
                    pointArrayList1.add(new Point(evt.getX(), evt.getY()));
                    if (pointArrayList1.size() == 3) {
                        ensembleFigure.add(new Triangle(pointArrayList1.get(0), pointArrayList1.get(1),pointArrayList1.get(2)));
                ...
                case "Cercle":
                    ...
                    if (pointArrayList1.size() == 2) {
                        ensembleFigure.add(new Cercle(pointArrayList1.get(0), pointArrayList1.get(1)));
                        pointArrayList1.clear();
                    }
                    break;
                case "Polygone":
        ...
        if (click.isSelected()) {
            switch (ajouteFigure) {
                ...
                case "Cercle":
                    ...
                    case "Triangle":
                    if (pointArrayList1.size() == 3) {
                        Triangle tr = new Triangle(pointArrayList1.get(0), pointArrayList1.get(1),pointArrayList1.get(2));
                        tr.paint(displayPanel);
                        ensembleFigure.add(tr);
                        pointArrayList1.clear();
                    }
                    if (pointArrayList1.size() == 1) {
                        Point p = new Point(evt.getX(), evt.getY());
                        Segment s1 = new Segment(pointArrayList1.get(0), p);
                        Trarr.add(s1);
                        Trarr.get(0).depaint(displayPanel);
                        s1.paint(displayPanel);
                        if (Trarr.size() == 2) {
                            Trarr.remove(0);
                        }
                        ...
                        if(pointArrayList1.size() == 2){
                            Segment s =new Segment(pointArrayList1.get(0), pointArrayList1.get(1));
                            s.paint(displayPanel);
                            Point p1 = new Point(evt.getX(), evt.getY());
                            Segment s2 = new Segment(pointArrayList1.get(1), p1);
                            Trarr.add(s2);
                            Trarr.get(0).depaint(displayPanel);
                            s2.paint(displayPanel);
                        if (Trarr.size() == 2) {
                            Trarr.remove(0);
                        }
                        }
                 break; 
                    case "Polygone":
                        if(b){
                        if (pointArrayList1.size()%2==1) {
                        if(k>2){
                        Segment s =new Segment(pointArrayList1.get(k-2), pointArrayList1.get(k-1));
                        ensembleFigure1.add(s);
                        ensembleFigure1.paint(displayPanel);
                        }
                        Point p = new Point(evt.getX(), evt.getY());
                        Segment s1 = new Segment(pointArrayList1.get(pointArrayList1.size()-1), p);
                        Polygarr.add(s1);
                        Polygarr.get(0).depaint(displayPanel);
                        s1.paint(displayPanel);
                        if (Polygarr.size() == 2) {
                            Polygarr.remove(0);
                        }
                           ...
                           if(pointArrayList1.size()%2==0 && pointArrayList1.size()!=0){
                            Segment s =new Segment(pointArrayList1.get(k-2), pointArrayList1.get(k-1));
                            ensembleFigure1.add(s);
                            ensembleFigure1.paint(displayPanel);
                            Point p1 = new Point(evt.getX(), evt.getY());
                            Segment s2 = new Segment(pointArrayList1.get(k-1), p1);
                            Polygarr.add(s2);
                            Polygarr.get(0).depaint(displayPanel);
                            s2.paint(displayPanel);
                        if (Polygarr.size() == 2) {
                     ...
                     case "Polyligne":
                        ...
                        if(k>2){
                        Segment s =new Segment(pointArrayList1.get(k-2), pointArrayList1.get(k-1));
                        ensembleFigure2.add(s);
                        ensembleFigure2.paint(displayPanel);
                        }
                        Point p = new Point(evt.getX(), evt.getY());
                        Segment s1 = new Segment(pointArrayList1.get(pointArrayList1.size()-1), p);
                        Polylarr.add(s1);
                        Polylarr.get(0).depaint(displayPanel);
                        s1.paint(displayPanel);
                        if (Polylarr.size() == 2) {
                            Polylarr.remove(0);
                        }
    ...
    private void displayPanelKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_displayPanelKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER && ajouteFigure.equals("Polygone")) {
            k=0;
            b=false;
            ensembleFigure.add(new Polygone(pointArrayList1));
            pointArrayList1.clear();
            b=true;
        ...
        if (evt.getKeyCode() == KeyEvent.VK_ENTER && ajouteFigure.equals("Polyligne")) {
            k=0;
            b=false;
            ensembleFigure.add(new Polyligne(pointArrayList1));
            pointArrayList1.clear();
            b=true;
    ...
    private void effacerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_effacerActionPerformed
        ...
        ensembleFigure1.clear();
        ensembleFigure2.clear();
        displayPanel.paint(displayPanel.getGraphics());
        ensembleFigure.paint(displayPanel);
